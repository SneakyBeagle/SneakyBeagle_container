...
git clone $GITHUB/diego-treitos/linux-smart-enumeration.git && cd diego-treitos/linux-smart-enumeration.git && chmod 700 lse.sh && cd $WORKDIR
git clone $GITHUB/411Hall/JAWS.git
git clone $GITHUB/bitsadmin/wesng.git && cd bitsadmin/wesng.git && python3 wes.py --update && cd $WORKDIR
